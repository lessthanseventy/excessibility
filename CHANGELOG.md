...
### Changed
- **Release workflow refuses arbitrary refs and resumes partial releases** ([#187](https://github.com/lessthanseventy/excessibility/issues/187)). The #176 manual release path was deliberate but did not enforce the conditions its comments assumed: `workflow_dispatch` could tag any dispatched branch/tag, the job never verified CI succeeded for the exact SHA, its final gate ran a reduced local `mix test` (no PostgreSQL service, no browser setup, DB-gated #175 regression excluded), and a transient failure after the tag push left a partial release that a rerun could not complete. `release.yml` now (1) refuses any ref except `main`; (2) fetches `origin/main` and requires the release `HEAD` to still be its exact tip; (3) verifies a **successful** CI *and* Version Check run exists for that exact SHA via the GitHub API (minimal `actions: read` scope) — so the real PostgreSQL + browser coverage that ran there is the release prerequisite, not a reduced local invocation; (4) refuses to release unless CHANGELOG has a dated `## [<version>] - YYYY-MM-DD` section **and** an empty `[Unreleased]`, so shipped fixes are never misclassified as unreleased; and (5) treats a tag that already targets the exact release SHA as a resumable rerun that (re)creates the missing GitHub release (idempotent), while a tag on any other SHA still aborts. Tags remain immutable and are never force-moved; `v0.18.1` is untouched. A new `test/release_workflow_test.exs` statically asserts these guards and that every released CHANGELOG section is ISO-dated, so a future edit that drops a guard fails in CI. Follow-up to #176.
- **Release flow no longer tags on every main push; orphaned `v0.19.0` tag resolved** ([#176](https://github.com/lessthanseventy/excessibility/issues/176)). [#168](https://github.com/lessthanseventy/excessibility/issues/168) hardened `version-tag.yml` to fail a main push when an existing `v$VERSION` tag did not target `HEAD`, but that also failed a corrected release commit (whose only delta may be outside the Hex package surface) and made re-tagging a version impossible. It is replaced by two workflows: `version-check.yml` (validation only — on push to main it checks README consistency and, when `v$VERSION` is already tagged, that the declared **Hex package surface** (the files `mix hex.build` ships) is unchanged; it never fails merely because `HEAD` diverges from a tag on non-package files, and never creates or moves tags) and `release.yml` (a deliberate `workflow_dispatch` path, run after the release PR merges and CI is green: it refuses to move an existing tag, then creates the tag and GitHub release; its release preconditions were subsequently hardened in [#187](https://github.com/lessthanseventy/excessibility/issues/187)). The already-public but unshipped `v0.19.0` tag (never published to Hex, no GitHub release) was resolved via **Option B**: the abandoned remote tag was deliberately deleted so the corrected line ships as `0.19.0`. `v0.18.1` and its Hex release remain immutable.

### Fixed
- **Cross-snapshot `content_change_without_live_region` no longer false-positives on full-page navigations** ([#191](https://github.com/lessthanseventy/excessibility/issues/191)). `SnapshotDiff.scan_sequence/2` (and the file-based `scan_files/2`) diffed *every* consecutive snapshot pair within a test and flagged any changed region outside an `aria-live`/`role=alert|status|log`/`<output>` as an un-announced status message (WCAG 4.1.3). That is correct for in-place LiveView patches, but journey-style tests snapshot several **different** views in one test — consecutive pairs there are separate pages, not a DOM patch — so every navigation produced a finding localized to a layout container (`div.mx-auto`, `body`, `header`…), and the only "fix" that satisfies the rule (wrapping the main content in `aria-live`) would harm screen-reader UX by re-reading the whole page on every route change. One real app saw 41 such findings, all pure navigation noise. A same-view identity guard now suppresses a pair **only on positive proof of navigation**: both snapshots carry a LiveView root id (`data-phx-main`/`data-phx-session`/`data-phx-root-id`, per-mount and patch-stable) and the ids differ — a fresh `live/2` mount, not a patch. In-place patches (same root id) are still flagged, and indeterminate identity (controller/static snapshots, plain fragments) is diffed exactly as before, so no real finding is silently dropped. The guard is DOM-based, so a LiveView rendered through Wallaby is covered too. `diff/3` and `live_region_findings/3` (the single-pair baseline compare in `Excessibility.Review`) are unchanged.
